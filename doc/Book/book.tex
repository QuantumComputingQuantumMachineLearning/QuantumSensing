...
% Chapter 2 FPM
\chapter{Quantum Mechanics for Quantum Sensing 1 (closed systems \& projective measurements)}
\abstract{
Here we review the basic quantum mechanics needed for sensing. It begins with state descriptions (pure and mixed) and the density-operator formalism, showing how any pure state yields a projector and how measurement probabilities are computed from the density matrix. The concept of projective measurement is presented (projective measurements in the energy basis, Born-rule probabilities, and state collapse). The chapter then introduces common qubit operations via Hermitian and unitary operators (notably the Pauli matrices and simple 2×2 gates) and how they flip or phase-shift basis states. Time evolution is treated next: the Schrödinger equation and its formal unitary solution $U\left(t\right)=\operatorname{e}^{-iHt/ \hbar } $ are derived, and the role of Hermitian (energy) and unitary operators is emphasized. The Heisenberg and interaction pictures are briefly discussed. The chapter concludes with remarks on state preparation and common quantum gates (Hadamard, phase, etc.). Overall, the chapter consolidates the quantum formalism needed to model sensors as quantum systems.
}
%\input{chapter2}
\bibliographystyle{spphys}
...
% Chapter 3 FPM
\chapter{Quantum Mechanics for Quantum Sensing 2 (open systems \& generalized measurements)}
\abstract{
Here we develop the quantum dynamics background beyond unitary dynamics and projective measurements, introducing several important concepts (operations, POVMs, effects) and showing how realistic detectors are modeled beyond ideal projectors and how a POVM can be mapped to a projective measurement on an enlarged space (Naimark’s dilation theorem). We then discuss a general classification of measurements (type I vs type II, efficient, complete, sharp, backaction-evading, weak, etc.). We then present some examples of concrete measurement setups: photodetection, homodyne detection, etc. 
}
%\input{chapter3}
\bibliographystyle{spphys}
...
% Chapter 8 ?
\chapter{Quantum Sensing Devices: sensing beyond em}
\abstract{This chapter treats signals beyond electromagnetic fields: inertial and gravitational sensing (acceleration, rotation, gravity gradients) with light-pulse atom interferometers and levitated optomechanics; force, displacement, and torque detection using cavity/levitated mechanical sensors and magnetic force resonant microscopy; time and frequency metrology with trapped-ion and neutral-atom clocks; and thermometry/calorimetry using solid-state defects, qubits, superconducting (TES/KIDs), and nanomechanical platforms. As in the previous chapter strong emphasis is placed on translating general control of these systems to practical readout of signals (non-EM here).}
%\input{chapter8}
\bibliographystyle{spphys}
...
\chapter{Quantum Sensor Networks and Distributed Sensing} %MHJ
...
\appendix 
\chapter{Basic Mathematics for Quantum Sensing, perhaps appendix material} %MHJ
